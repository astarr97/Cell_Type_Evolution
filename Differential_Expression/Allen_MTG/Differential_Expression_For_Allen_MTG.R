...
### NOTE: THIS ASSUMES THAT THE GENE NAMES ARE SORTED IDENTICALLY IN EACH FILE IN EACH SPECIES ###
...
config_DESeq <- 0
for (x in list.files()) {
  if (grepl("Config", x, fixed = TRUE) & (grepl("Chimp", x, fixed = TRUE) | grepl("Gorilla", x, fixed = TRUE) | grepl("Rhesus", x, fixed = TRUE) | grepl("Marmoset", x, fixed = TRUE))) {
    config_DESeq <- x
    print(config_DESeq)
    ase <- FALSE
    outfile = str_replace(config_DESeq, "Config", "DESeq2")
    ...
    #Do the DESeq2 thing.
    res <- results(dds)
    if (type_2 == "rhesus" | type_2 == "orangutan" | type_2 == "mouse" | type_2 == "marmoset") {
      get <- paste("cond_",type_2,"_vs_",type_1, sep = "")
      print("Sign Swap Needed")
    } else {
    ...
    #plotMA(resLFC)
    #dev.off()
    output_file <- cbind(repVals$V1, resLFC[,"log2FoldChange"],resLFC[,"pvalue"],resLFC[,"padj"],resLFC[,"padj_Mine"])
    colnames(output_file) <- c("Gene", "log2FoldChange","pvalue","padj","padj_mine")
    ...
    for (file in files) {
      if (grepl("HSB", file, fixed=TRUE) | grepl("Human", file, fixed=TRUE)){
        geneinfo = "human_gene_length.txt"
      } else if (grepl("PTB", file, fixed=TRUE) | grepl("Chimp", file, fixed=TRUE)){
        geneinfo = "chimp_gene_length.txt"
      } else if (grepl("RMB", file, fixed=TRUE) | grepl("Rhesus", file, fixed=TRUE)) {
      ...
      } else if (grepl("CJB", file, fixed=TRUE) | grepl("Marmoset", file, fixed=TRUE)) {
